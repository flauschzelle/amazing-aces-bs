...
def events
    blk = -> { @items.find_all("/**/*").select{|i| i[:eventdate] && i[:eventdate].jd >= Date::today.jd - 30}.sort_by{|i| i[:eventdate].jd} }
    if @items.frozen?
        @events ||= blk.call
    else
        blk.call
...
def nextevents
    blk = -> { @items.find_all("/**/*").select{|i| i[:eventdate] && i[:eventdate].jd >= Date::today.jd - 30}.sort_by{|i| i[:eventdate].jd}.take(3) }
    if @items.frozen?
        @nextevents ||= blk.call
    else
        blk.call
...
def abstract_for item
    if item[:abstract]
        ...
        content.gsub!(/[*"#]/,"") # remove italic and bold markers, quotations and headline markers
        content.strip!
