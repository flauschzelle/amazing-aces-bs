...
def events
    blk = -> { @items.find_all("/**/*").select{|i| i[:eventdate] && i[:eventdate].jd >= Date::today.jd - 14}.sort_by{|i| i[:eventdate].jd} }
    if @items.frozen?
        @events ||= blk.call
    else
        blk.call
...
def nextevents
    blk = -> { @items.find_all("/**/*").select{|i| i[:eventdate] && i[:eventdate].jd >= Date::today.jd - 14}.sort_by{|i| i[:eventdate].jd}.take(3) }
    if @items.frozen?
        @nextevents ||= blk.call
    else
        blk.call
